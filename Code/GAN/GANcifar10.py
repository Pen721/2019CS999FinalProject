...
torch.save(generator.state_dict(), 'models/c10/G')
discriminator = Discriminator()
torch.save(discriminator.state_dict(), 'models/c10/D')
...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(gen_imgs.data[:64], "images/gan/cifar102/%d.png" % (k), nrow=8, normalize=True)
            k=k+1
...
images = []
for i in range(k):
    img = imageio.imread('images/gan/cifar102/%d.png' % i)
    images.append(img)
imageio.mimsave('images/gan/cifar102/evo.gif', images, fps=5)
